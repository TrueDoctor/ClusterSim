namespace ClusterSim.ClusterLib.Analysis
{
    ...
    public class GnuPlot
    {
        ...
        private static void Plot(List<StoredPlot> storedPlots)
        {
            ...
            for (int i = 0; i < storedPlots.Count; i++)
            {
                ...
                switch (p.PlotType)
                {
                    ...
                    case PlotTypes.SplotZ:
                        ...
                        break;
                    case PlotTypes.SplotXYZc:
                        WriteData(p.X, p.Y, p.Z, p.I, GnupStWr, false);
                        GnupStWr.WriteLine("e");
                        break;
                }
            }
        ...
        public static void WriteData(double[] x, double[] y, double[] z, double[] I, StreamWriter stream, bool flush = true)
        {
            int m = Math.Min(x.Length, y.Length);
            m = Math.Min(m, z.Length);
            for (int i = 0; i < m; i++)
            {
                if (i > 0 && x[i] != x[i - 1])
                    stream.WriteLine("");
                stream.WriteLine(x[i] + " " + y[i] + " " + z[i] + " " + I[i]);
            }
